        ...
        <form @submit.prevent="saveStudent" class="space-y-4">
          ...
          <template v-if="!isEditing">
            <div class="bg-blue-50 border border-blue-100 rounded-[1px] p-3">
              ...
              <p class="text-[9px] text-blue-400 poppins mt-1">The student will receive an email with instructions to set their own password.</p>
            </div>
...
const fetchStudents = async () => {
  ...
  try {
    ...
    const [resS, resStats] = await Promise.all([
      api.get('/students/', { params }),
      api.get('/students/stats/', { params })
    ])
    
    students.value = resS.data.results || resS.data
    totalRecords.value = resS.data.count || students.value.length
...
const openAddModal = () => {
  ...
  form.value = { username: '', student_number: '', id_number: '', first_name: '', last_name: '', mi: '', email: '', phone_number: '', gender: '', course: 'BSHM', year_level: '1' }
  formError.value = ''
...
const editStudent = (s) => {
  ...
  form.value = { username: '', student_number: s.student_number || '', id_number: s.student_number || '', first_name: s.first_name || '', last_name: s.last_name || '', mi: s.mi || '', email: s.email || '', phone_number: s.phone_number || '', gender: s.gender || '', course: s.course || 'BSHM', year_level: s.year_level || '1' }
  formError.value = ''
  showModal.value = true
}
...
const saveStudent = async () => {
  ...
  try {
    if (isEditing.value) {
      ...
      alert('Student account created successfully! An email has been sent to the student with instructions to set their password.')
    }
    await fetchStudents()
    closeModal()
  } catch (err) {
...
const resetPassword = async (s) => {
  const confirmed = await modalStore.confirm({
    ...
    message: `Send a password reset email to ${s.full_name} (${s.email})? They will receive instructions to set a new password.`,
    variant: 'warning',
...
const prevPage = () => { if (currentPage.value > 1) { currentPage.value--; fetchStudents() } }
const nextPage = () => { if (currentPage.value < totalPages.value) { currentPage.value++; fetchStudents() } }
const goToPage = (p) => { if (p !== '...') { currentPage.value = p; fetchStudents() } }
