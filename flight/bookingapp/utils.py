...
def evaluate_passenger_information_quality(booking, activity):
    ...
    (This is essentially your existing implementation, lightly adapted)
    """
...
def evaluate_correct_passenger_details(activity, booking_details, penalty_extra_multiplier=0.5):
    ...
    Returns a dict:
    {
      'details': [ { 'expected': {...}, 'student': {...}, 'field_results': {...}, 'score': x, 'possible': y } , ... ],
    ...
    penalty_extra_multiplier: fraction of one passenger's possible points to deduct per extra passenger.
    """
    print("=== EVALUATE CORRECT PASSENGER DETAILS ===")

    expected_qs = list(activity.passengers.all())  # ActivityPassenger instances in order
    student_passengers = booking_details.get('passengers', [])  # list of dicts
    ...
    for idx, expected in enumerate(expected_qs):
        ...
        for field, pts in field_weights.items():
            ...
            if field == 'date_of_birth':
                # Both empty -> treat as incorrect; both non-empty compare raw strings (they are YYYY-MM-DD from get_booking_details)
                is_correct = (expected_val == student_val and expected_val != '')
    ...
    # Extra student passengers -> penalty
    if student_count > expected_count:
        extra = student_count - expected_count
        penalty_points = extra * passenger_possible * penalty_extra_multiplier
        print(f"Penalty for {extra} extra passengers: {penalty_points} points (multiplier {penalty_extra_multiplier})")
...
def get_detailed_comparison(activity, booking, booking_details, submission=None):
    ...
    # Evaluate passenger information quality (completeness)
    passenger_info_quality_score = evaluate_passenger_information_quality(booking, activity)
